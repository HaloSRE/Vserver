...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}Dalichuqiji@123$none)"): " sspass
		[ -z "$sspass" ] && sspass="Dalichuqiji@123"
		case $sspass in
		*[/$]*)
			echo
...
backup_config() {
	...
	if [[ $shadowsocks ]]; then
		sed -i "42s/=/=true/; 45s/=6666/=$ssport/; 48s/=console.cn.com/=$sspass/; 51s/=chacha20-ietf/=$ssciphers/" $backup
	fi
	[[ $v2ray_transport == [45] || $v2ray_transport == 33 ]] && sed -i "36s/=console.cn.com/=$domain/" $backup
